...
function Content() {
  ...
  return (
    <div className="w-full  h-full sm:gap-1 scrollbar-none  px-2 sm:px-48 sm:py-5 flex flex-col justify-between py-5 ">
      {
        //this div will disable if we have chats
      
      ChatsList.length<=0&&<div className="w-fit h-full flex flex-col gap-16 sm:gap-10">
      ...
      <div className="w-full h-full flex flex-col gap-16 overflow-y-scroll scrollbar-none">
        
{
  ChatsList.length > 0 ? (
    ChatsList.map((each) => (
      ...
      <div className={` flex flex-col h-96 sm:h-${ChatsList.length>0? `96 mt-24`:  `full`} items-center px-4 gap-3`}>
        <div className="w-full h-fit sm:h-16 bg-slate-100 rounded-lg sm:rounded-full gap-5  flex flex-col sm:flex-row justify-start sm:justify-center first-letter: px-7 py-5 items-center">
            <textarea value={prompt} onChange={(e)=>setprompt(e.target.value)}  placeholder="Enter a prompt here" className=" scrollbar-none scrollbar-thumb-rounded scrollbar-thumb-gray-500 resize-none grid-rows-2 sm:grid-rows-4 w-full h-fit sm:h-full placeholder-gray-700 bg-transparent  outline-none"></textarea>
            ...
            <button type="button" onClick={(e)=>OnSendM(e)} className="w-6 h-5">
  <img className='w-8 h-6' src={assets.send_icon} alt="send button" />
</button>

            </div>
           
        </div>
        <h6 className="text-xs h-fit">Gemini may display inaccurate info, including about people, so double-check its responses. Your privacy & Gemini Apps</h6>
      </div>
    </div>
  );
}
